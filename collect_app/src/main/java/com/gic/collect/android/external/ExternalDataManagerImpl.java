...
public class ExternalDataManagerImpl implements ExternalDataManager {
    ...
    public ExternalSQLiteOpenHelper getDatabase(String dataSetName, boolean required) {
        ...
        if (sqLiteOpenHelper == null) {
            if (mediaFolder == null) {
                String msg = Collect.getInstance().getString(com.gic.collect.android.R.string.ext_not_initialized_error);
                Timber.e(msg);
                if (required) {
                ...
                if (!dbFile.exists()) {
                    String msg = Collect.getInstance().getString(
                            com.gic.collect.android.R.string.ext_import_csv_missing_error, dataSetName, dataSetName);
                    Timber.e(msg);
                    if (required) {
                        throw new ExternalDataException(msg);
                    } else {
