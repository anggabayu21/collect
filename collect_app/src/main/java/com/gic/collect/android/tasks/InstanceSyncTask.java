...
import static com.gic.collect.android.provider.InstanceProviderAPI.InstanceColumns;
...
public class InstanceSyncTask extends AsyncTask<Void, String, String> {
    ...
    protected String doInBackground(Void... params) {
        ...
        currentStatus = Collect.getInstance().getString(com.gic.collect.android.R.string.instance_scan_completed);
        try {
            ...
            if (instancesPath.exists() && instancesPath.isDirectory()) {
                ...
                if (counter > 0) {
                    currentStatus += String.format(
                            Collect.getInstance().getString(com.gic.collect.android.R.string.instance_scan_count),
                            counter);
                }
            }
        } finally {
